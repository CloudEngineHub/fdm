    ...
    for topic, msg, t in bag.read_messages(topics=args.topics):
        ...
        if topic_type == AnymalState._type:
            ...
            try:
                ...
                transform = tf_buffer.lookup_transform(args.unified_frame, msg.info.header.frame_id, msg.info.header.stamp)
            except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
                rospy.logerr(f"Transform failed of msg {topic_type} at time {t.to_time()}. Disregard message")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-bf",
        "--bag_file",
        default="/media/pascal/T7 Shield/FDMData/GrandTour/2024-11-14-14-36-02_forest_kaeferberg_entanglement/merged.bag",
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=["/elevation_mapping/elevation_map_raw", "/anymal_low_level_controller/actuator_readings", "/state_estimator/anymal_state", "/dlio/lidar_map_odometry", "/twist_mux/twist"],  # "/path_planning_and_following/twist", "/clicked_point", "/planner_node/path", 
        help="Topics.",
    )
    parser.add_argument(
