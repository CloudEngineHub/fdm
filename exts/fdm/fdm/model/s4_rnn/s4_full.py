...
class Kernel(nn.Module):
    ...
    def __init__(
        ...
        channels: int = 1,
        l_max: int | None = None,
        lr: float | Mapping | None = None,
        wd: float | Mapping | None = 0.0,
        **kwargs,
    ):
        """General interface.
...
class SSMKernel(Kernel):
    ...
    def init_dt(self):
        ...
        if self.deterministic:  # Meant for debugging
            ...
            # Initialize log dt
            inv_dt = torch.rand(*shape, dtype=self.dtype) * (math.log(self.dt_max) - math.log(self.dt_min)) + math.log(
                self.dt_min
            )
    ...
    def init_ssm_dplr(self):
        ...
        assert self.n_ssm % B.size(-2) == 0 and self.n_ssm % P.size(-2) == 0 and self.n_ssm % A.size(-2) == 0
...
class S4Block(nn.Module):
    ...
    def forward(self, x, lengths=None, **kwargs):  # absorbs return_output and transformer src mask
        ...
        if lengths is not None:
            ...
            mask = torch.where(torch.arange(L, device=lengths.device)[:, None] < lengths[:, None, None], 1.0, 0.0)
            x = x * mask
