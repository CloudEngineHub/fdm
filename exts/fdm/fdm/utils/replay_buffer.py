...
class ReplayBuffer:
    ...
    def data_collection_interval(self):
        # define after how many steps (decimation x physics_dt) the data should be collected
    ...
    def history_collection_interval(self):
        ...
        history_collection_interval = self.data_collection_interval / self.model_cfg.history_length
        assert history_collection_interval >= 1, (
        ...
        return history_collection_interval
    ...
    def add(
        ...
        colliding_envs[self.env_step_counter < self.data_collection_interval] = False
    ...
    def _update_local_history_buffers(
        ...
        updatable_envs = (self.env_step_counter % self.history_collection_interval).type(torch.int) == 0  # noqa: E721
    ...
    def _update_full_trajectory_buffers(
        ...
        updatable_envs = (self.env_step_counter % self.data_collection_interval).type(torch.int) == 0  # noqa: E721
        # filter if step is 0
        updatable_envs[self.env_step_counter == 0] = False
        # enable if environment is colliding
        updatable_envs[colliding_envs] = True
        updatable_envs[self.env_step_counter == self.data_collection_interval] = ~colliding_envs[
            self.env_step_counter == self.data_collection_interval
        ].to(self.device)
        # don't update if robot has not touched the ground yet (initial falling period after reset)
        updatable_envs[~feet_contact] = False
